...
2. **Format instances.** The fat reader becomes the host plus `pdf.wasm`, `text.wasm`, and `md.wasm`. The shelf is `library.wasm`, and it does not run beside the host. One slot. Switching format drops that format instance. Returning to the shelf drops every book instance, not only the slot, and then drops the host. Sidebar and appearance talk across the bridge. Search-index retention is deleted here, because its reason is gone. The spec is `docs/phase-2-format-instances.md`.
