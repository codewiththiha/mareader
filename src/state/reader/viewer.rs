...
//! and the motion projection ([`Motion`], `reader-core`'s) that says which of
//! the reader's movements are allowed to animate.
...
// The motion projection is the reader's format-agnostic policy, so it lives in
// `reader-core` beside the settings it is projected from; the shell's chrome
// reads it too, and neither half of the app may own it. Re-exported here
// because this module is where the reader's signals are named.
pub use reader_core::motion::Motion;
