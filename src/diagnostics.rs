...
use crate::memory::wasm_heap_bytes;
...
/// [`crate::memory`] logs through here so the chart and the snapshot see the
/// same number.
...
fn install_web(reader_runtime_live: impl Fn() -> bool + 'static) {
    ...
    // The surface lives as long as the app; into_js_value hands the
    // closure's lifetime to the window property, which is exactly the
    // intent. The closure's captures are the Copy app state and the JSON
    // builder.
