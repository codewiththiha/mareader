...
use app_state::boundary::ReadPoint;
...
pub fn save_cover(path: &str, image: app_state::state::covers::CoverImage) {
    ...
    if !map.contains_key(path) && map.len() >= app_state::state::covers::COVER_CAP {
