...
pub enum ReadingData {
    ...
    /// puts it back on the row it lands (`crate::storage::kept`).
    Keep,
...
pub fn purge_books(state: AppState, row_ids: &[String], data: ReadingData) {
...
fn settle_reading_data(book: &Book, data: ReadingData) {
    match data {
        ReadingData::Keep => {
            let marks = crate::storage::take_gloss(&book.id);
            crate::storage::kept::remember(book, marks);
        }
        ReadingData::Delete => {
            crate::storage::kept::forget(book);
