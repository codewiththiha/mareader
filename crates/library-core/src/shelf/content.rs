...
pub fn content_kind(rows: &[Row], shelf: &Shelf) -> ContentKind {
    content_kind_for_ids(rows, &shelf.books)
}

/// Same as [`content_kind`] but over an explicit list of member ids. Shared by
/// the recursive walk.
fn content_kind_for_ids(rows: &[Row], member_ids: &[String]) -> ContentKind {
    ...
    for member_id in member_ids {
        let Some(row) = find_row(rows, member_id) else {
            continue;
        };
        let book = match row {
            Row::Book(b) => b,
            Row::Link { target, .. } => {
                ...
                let Some(Row::Book(b)) = find_row(rows, target) else {
                    continue;
                };
                b
            }
