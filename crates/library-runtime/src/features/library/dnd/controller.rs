...
impl DragController {
    pub fn install(state: crate::context::LibraryContext) -> Self {
        let this = Self {
        ...
        Effect::new(move |_| {
            ...
            if sinkable && let Some(rect) = this.registry.rect_of(&target) {
                ...
                if let Ok(sunk) = set_timeout_with_handle(
                    move || {
                        ...
                        if dwell.as_ref() != Some(&still) {
                            return;
                        }
                        let _ = this.sink.try_set(Some(spot));
                        let _ = this.sink_rect.try_set(Some(bounds));
                    },
            ...
            let Ok(folded) = set_timeout_with_handle(
                move || {
                    ...
                    if this.dwell.try_get_untracked().is_none() {
                        return;
                    }
                    let _ = this.dwell.try_set(true);
                    this.refresh();
                },
