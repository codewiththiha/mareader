...
pub fn apply(state: crate::context::LibraryContext, effect: DropEffect, payload: DragPayload) {
    if payload.is_empty() {
        return;
    }
...
fn land_folders(state: crate::context::LibraryContext, folders: &[String], to: &str) {
    if to == ALL_SHELF {
