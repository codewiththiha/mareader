//! The reader rail's composition: the aside and its four slots, written once
//! and mounted from one of two places.
//!
//! The two places are not a stylistic choice — see the rail family's
//! `overlay.rs` for the stacking-context story that puts the floating rail
//! outside `.reader-bg`. [`crate::features::root::ReaderRoot`] mounts
//! [`crate::components::rail::push::PushRail`] and
//! [`crate::components::rail::overlay::OverlayRail`], each self-gating on the
//! shell controller's layout; this composition renders identically inside
//! either.
//!
...
pub(crate) fn ReaderRail(
    /// The open document's slice: what the rail's panels are a view of.
    reader: ReaderState,
    /// The library's cover map, handed in for the identity row's thumbnail.
    /// The reader never reaches the library itself — the row is the one place
    /// a book's shelf face shows up while it is open.
    covers: RwSignal<CoverMap>,
    /// The shell's layout truth, owned by the reader root so both mount points
