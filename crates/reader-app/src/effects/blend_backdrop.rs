...
use reader_core::settings::{LayoutSettings, Settings};
...
/// first.
pub fn paper_settings(settings: RwSignal<Settings>) {
...
/// Geometry → the session. Called once from the reader root, alongside the
/// other reader effects; see the module doc for why this half waits for the
/// reader.
pub fn blend_backdrop(state: ReaderState, settings: RwSignal<Settings>) {
    let viewer = state.viewer;
    let heights = state.document.content.metrics.css_heights;
