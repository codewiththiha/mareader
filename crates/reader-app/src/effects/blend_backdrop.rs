...
//! [`blend_backdrop`] (geometry) stays with the reader route, where the
...
/// Geometry → the session. Called once from the reader route, alongside the other
/// reader effects; see the module doc for why this half waits for the reader.
pub fn blend_backdrop(reader: ReaderState, settings: RwSignal<Settings>) {
    let viewer = reader.viewer;
    let heights = reader.document.content.metrics.css_heights;
