...
//! `reader.viewer.selected_pages`, which
...
pub fn page_selection(reader: ReaderState) {
    use_raw_event(ui_kit::events::SELECTION_PAGES_EVENT, move |detail| {
        match parse_selection(detail) {
            Some((first, last)) => {
                let total = reader.document.num_pages.get_untracked().max(1);
                let f = first.clamp(1, total);
                let l = last.clamp(1, total);
                reader.viewer.selected_pages.set(Some((f.min(l), f.max(l))));
            }
            None => reader.viewer.selected_pages.set(None),
        }
    });
}
