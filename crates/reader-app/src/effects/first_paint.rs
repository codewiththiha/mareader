...
/// it.
fn release_when_painted(reader: ReaderState) {
    let r = reader;
    Effect::new(move |_| {
...
/// sampled).
fn release_if_never_painted(reader: ReaderState) {
    let r = reader;
...
pub fn first_paint_gate(reader: ReaderState) {
    release_when_painted(reader);
    release_if_never_painted(reader);
}
