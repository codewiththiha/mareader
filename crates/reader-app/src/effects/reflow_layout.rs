...
pub fn reflow_layout(state: ReaderState, vertical: Virtualizer) {
    Effect::new(move |_| {
        ...
        let cuts = state.document.content.reflow.cuts.get();
        ...
        if state.document.content.metrics.heights_agree(&sizes) {
            return;
        }
        let metrics = state.document.content.metrics;
