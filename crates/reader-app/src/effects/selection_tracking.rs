...
//! `reader.ai_selection`: `detail` carries the text/context, `anchor`
...
use ui_kit::primitives::hooks::use_custom_event::use_raw_event;
...
fn anchor_for(detail: &SelectionDetail, reader: ReaderState) -> Option<PageAnchor> {
...
pub fn selection_tracking(reader: ReaderState) {
    use_raw_event(ui_kit::events::SELECTION_DETAIL_EVENT, move |detail| {
        match parse_selection_detail(detail) {
            Some(selection) => {
                let anchor = anchor_for(&selection, reader);
                reader.ai_selection.anchor.set(anchor);
                reader.ai_selection.detail.set(Some(selection));
                // A new selection supersedes any open explanation.
                reader.ai_selection.popover_open.set(false);
            }
            None => {
                reader.ai_selection.anchor.set(None);
                reader.ai_selection.detail.set(None);
                reader.ai_selection.popover_open.set(false);
            }
        }
    });
