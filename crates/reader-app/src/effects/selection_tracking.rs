...
//! `state.ai_selection`: `detail` carries the text/context, `anchor`
...
fn anchor_for(detail: &SelectionDetail, state: ReaderState) -> Option<PageAnchor> {
    let reader = state;
...
pub fn selection_tracking(state: ReaderState) {
    use_raw_event(ui_kit::events::SELECTION_DETAIL_EVENT, move |detail| {
        match parse_selection_detail(detail) {
            Some(selection) => {
                ...
                state.ai_selection.anchor.set(anchor);
                state.ai_selection.detail.set(Some(selection));
                // A new selection supersedes any open explanation.
                state.ai_selection.popover_open.set(false);
            }
            None => {
                state.ai_selection.anchor.set(None);
                state.ai_selection.detail.set(None);
                state.ai_selection.popover_open.set(false);
            }
        }
    });
