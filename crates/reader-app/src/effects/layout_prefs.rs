...
use app_chrome::hooks::dom::html_style;
...
pub fn layout_prefs(
    reader: ReaderState,
    // The persisted layout the margin/gap dials resolve from.
    ...
    {
        let m = settings.with_untracked(|st| st.layout.page_margin);
    ...
    // document streams.
    {
        ...
        Effect::new(move |_| {
            let no_gap = settings.with(|st| st.layout.no_gap);
    ...
    // flip itself.
    {
        ...
        Effect::new(move |_| {
            let stored = settings.with(|st| st.layout.page_margin);
            ...
            // against (`crate::effects::mode_change`).
    ...
    Effect::new(move |_| {
        let pct = settings.with(|st| st.layout.column_width_pct);
