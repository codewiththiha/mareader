...
/// open.
pub fn install_reflow_measure(reader: ReaderState) {
    ...
    Effect::new(move |_| {
        ...
        let pct = reader.viewer.column_width_pct.get();
        ...
        if !heights_moved(&reflow.heights.get_untracked(), &merged) {
            ...
            recut_and_publish(reader, reflow, geo);
            return;
        }
...
fn recut_and_publish(
    ...
    if let Some(cut) = reflow.recut(reader.viewer.page.get_untracked(), reader.viewer.zoom.visual_scale(), geo) {
        reader.document.publish_cut(&cut);
        reader.viewer.page.set(cut.page);
    }
}
