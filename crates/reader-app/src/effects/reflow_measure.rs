...
/// open.
pub fn install_reflow_measure(state: ReaderState) {
    ...
    Effect::new(move |_| {
        ...
        let pct = state.viewer.column_width_pct.get();
...
fn recut_and_publish(
    ...
    if let Some(cut) =
        reflow.recut(state.viewer.page.get_untracked(), state.viewer.zoom.visual_scale(), geo)
    {
        state.document.publish_cut(&cut);
        state.viewer.page.set(cut.page);
    }
}
