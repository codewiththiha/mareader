...
/// Called once from the reader route, alongside `follow_watcher`.
///
...
pub fn fit_watcher(
    reader: ReaderState,
    // Read for auto-resize and the column dial; never written.
    ...
    Effect::new(move |_| {
        ...
        let _ = settings.with(|st| st.layout.column_width_pct);
        ...
        // the same frame.
        if settings.with(|st| st.layout.auto_resize) {
            let _ = vs.page.get();
        }
...
///
/// Called once from the reader route, alongside `fit_watcher`.
pub fn follow_watcher(reader: ReaderState, sidebar: RwSignal<SidebarMode>) {
    let vs = reader.viewer;
