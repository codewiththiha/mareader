...
//! stream itself measures — see `crate::effects::reflow_measure`).
//!
...
//! for the stream (see `crate::effects::navigation_sync`), because both
...
pub fn ReflowStreamLayout(
    ...
    // (`crate::effects::reflow_measure::ingest`), which debounces them into
    ...
    let column_ref: NodeRef<html::Div> = NodeRef::new();
    {
        ...
        Effect::new(move |_| {
            ...
            request_animation_frame(move || {
                ...
                crate::effects::reflow_measure::ingest(doc_id, scale, &batch);
            });
        });
    }
