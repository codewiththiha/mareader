...
pub(crate) fn SidebarHeader(
    ...
    // through context (see `features/root.rs`), so this header can open the same
                ...
                <IconButton
                    ...
                    on_click=move || {
                        if reader.search.visible.get() {
                            crate::effects::search::dismiss_search(reader);
                        } else {
                            crate::effects::search::resume_search(reader);
                        }
                    }
                />
            </Tooltip>
