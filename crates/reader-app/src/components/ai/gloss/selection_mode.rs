...
//!
//! Selection state itself lives on `state.gloss` so every page's
...
pub fn exit_selection(state: ReaderState) {
    state.gloss.selection_active.set(false);
    state.gloss.selected_marks.set(HashSet::new());
}
...
pub fn use_select_mode(state: ReaderState, ctrl: GlossController) -> SelectMode {
    let selecting = state.gloss.selection_active;
