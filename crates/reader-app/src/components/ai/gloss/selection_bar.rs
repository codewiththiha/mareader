...
use ui_kit::primitives::controls::button::{Button, ButtonTone, ButtonVariant};
use ui_kit::primitives::overlay::action_bar::ActionBar;
...
pub fn GlossSelectBar(
    ...
    let selected = reader.gloss.selected_marks;
    let marks = reader.gloss.marks;
    let page = reader.viewer.page;
    ...
    let remove = move |_| {
        ...
        let path = reader.document.path.get_untracked();
        park_undo(undo, removed, path);
        exit_selection(reader);
    };

    let done = move |_| exit_selection(reader);
