...
pub fn GlossAiPopover(
    reader: ReaderState,
    /// The persisted settings snapshot the card reads its density from.
    ...
    view! {
        ...
        <GlossSelectBar reader ctrl undo=sm.undo />
        <GlossContextMenu reader ctrl menu=sm.menu undo=sm.undo />
        <GlossUndoToast reader ctrl undo=sm.undo />
    }
}
