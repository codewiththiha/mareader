...
#[component]
pub fn GlossAiPopover(
    /// The open document's slice: every gloss signal the card reads and writes.
    reader: ReaderState,
    /// The persisted settings, for the one knob this surface paints by — the
    /// card's density. Read-only, and handed in rather than reached for.
    settings: RwSignal<Settings>,
) -> impl IntoView {
    // ── State machine hub ─────────────────────────────────────────────
    let ctrl = use_gloss_controller(reader);

    // ── Card targeting: anchor watch, viewport, spring, progress ──────
    let card = use_card_targeting(reader, ctrl);

    // ── Open/close plumbing ───────────────────────────────────────────
    use_open_listener(reader, ctrl);
    use_open_effect(reader, ctrl, card.watch, card.spring, card.viewport);
    use_ai_chunks(reader, ctrl);
    ...
    let density_sig = Signal::derive(move || settings.with(|s| s.gloss_density));
    ...
    view! {
        ...
        <GlossSelectBar state=reader ctrl undo=sm.undo />
        <GlossContextMenu state=reader ctrl menu=sm.menu undo=sm.undo />
        <GlossUndoToast state=reader ctrl undo=sm.undo />
    }
}
