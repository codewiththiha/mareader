...
use crate::state::{GlossSave, ReaderState};
...
pub(super) fn build_commands(
    ...
    let processing_id = state.gloss.processing_id;
    let marks = state.gloss.marks;

    // The marks' one write to storage, through the callback the shell
    // injected. There is no per-mark write: the stored shape is a document's
    // list, so every mutation persists the whole list, and a document that is
    // not open has nowhere to put it. The KEY is `crate::state::gloss_key`'s
    ...
    // cannot drift.
    let save = expect_context::<GlossSave>();
    let persist = move || {
        let key = crate::state::gloss_key(&state);
        if !key.is_empty() {
