...
pub(super) fn build_commands(
    ...
    let processing_id = reader.gloss.processing_id;
    let marks = reader.gloss.marks;
    ...
    // `crate::state::gloss_key`'s rather than the path, so a book of its own
    // writes the list its own reader reads — the load at open asks the same
    // question and the two cannot drift. The WRITE itself is the shell's
    // storage, injected as the [`crate::GlossSave`] context: persistence is
    // the shell's authority; the key and the list are the reader's.
    let persist = move || {
        let key = crate::state::gloss_key(&reader);
        if !key.is_empty() {
            save.run((key, marks.get_untracked()));
        }
    };
