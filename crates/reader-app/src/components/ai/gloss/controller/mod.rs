...
pub fn use_gloss_controller(reader: ReaderState) -> GlossController {
    // The persistence door is the shell's (`crate::GlossSave`, provided at
    // the route). Absent — a reader mounted with no shell above it, which
    // Phase 2's own build will be until its host wires one in — persistence
    // becomes a silent no-op rather than a panic.
    let save = use_context::<crate::GlossSave>().unwrap_or(Callback::new(|_| {}));
    ...
    GlossController {
        ...
        drag,
        cache,
        commands: commands::build_commands(reader, save, content, geometry, open, drag, cache),
    }
}
