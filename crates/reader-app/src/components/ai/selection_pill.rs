...
#[component]
pub fn SelectionPill(state: ReaderState) -> impl IntoView {
    let detail = state.ai_selection.detail;
    let popover_open = state.ai_selection.popover_open;
    ...
    let resolve = anchor_resolver(state, spot);
    ...
    let invalidate = if state.reflowable_now() {
        reflow_invalidation(state)
    } else {
        no_invalidation()
    };
    let watch = watch_page_anchor(
        Signal::derive(move || state.ai_selection.anchor.get()),
        resolve,
        state.viewer.zoom.display.into(),
        state.viewer.scroll_top.into(),
        state.viewer.page.into(),
        invalidate,
    );
    ...
    Effect::new(move |_| {
        if watch.exited.get() && detail.get().is_some() {
            detail.set(None);
            state.ai_selection.anchor.set(None);
        }
    });
            ...
            >
                <button
                    ...
                    on:click=move |_| {
                        ...
                        // owns this selection.
                        let captured = state.ai_selection.anchor.get_untracked();
                            ...
                            .or_else(|| {
                                if reflow {
                                    ...
                                    ReflowAnchorBridge {
                                        state,
                                        spot: None,
                                        mode: state.viewer.mode.get_untracked(),
                                    }
                                    .capture(state.viewer.zoom.visual_scale())
                                    .and_then(|pa| {
