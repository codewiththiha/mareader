...
#[component]
pub fn SelectionPill(reader: ReaderState) -> impl IntoView {
    let detail = reader.ai_selection.detail;
    let popover_open = reader.ai_selection.popover_open;
    ...
    let resolve = anchor_resolver(reader, spot);
    ...
    let invalidate = if reader.reflowable_now() {
        reflow_invalidation(reader)
    } else {
        no_invalidation()
    };
    let watch = watch_page_anchor(
        Signal::derive(move || reader.ai_selection.anchor.get()),
        resolve,
        reader.viewer.zoom.display.into(),
        reader.viewer.scroll_top.into(),
        reader.viewer.page.into(),
        invalidate,
    );
    ...
    Effect::new(move |_| {
        if watch.exited.get() && detail.get().is_some() {
            detail.set(None);
            reader.ai_selection.anchor.set(None);
        }
    });
            ...
            >
                <button
                    ...
                    on:click=move |_| {
                        ...
                        // owns this selection.
                        let captured = reader.ai_selection.anchor.get_untracked();
                            ...
                            .or_else(|| {
                                if reflow {
                                    ...
                                    ReflowAnchorBridge {
                                        state: reader,
                                        spot: None,
                                        mode: reader.viewer.mode.get_untracked(),
                                    }
                                    .capture(reader.viewer.zoom.visual_scale())
                                    .and_then(|pa| {
                                    ...
                                    capture_selection_mark(
                                        reader.viewer.zoom.visual_scale(),
                                        sel.text.clone(),
                                        sel.context.clone(),
                                    )
                                }
