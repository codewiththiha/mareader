...
//!
//! Shown only when a document is open, the rail is off, and — unless
//! "Always Show Label" is on — the titlebar is not visible (the bar contains
//! the name) and the name fits its budget.
//!
//! It lives with the viewer rather than with the bars because everything it
//! measures is the viewer's: the page host under the eyes, the slot it sits
//! in, and the document's own name. What it asks the window for — whether the
//! bar is showing, whether the rail is present — it asks through the two
//! contexts the shell provides (`TitleBarCtx`, `ShellController`), and both
//! reads are optional: a reader with no bar above it simply has no bar to
//! defer to.
//!
...
#[component]
pub fn FloatingDocumentTitle(
    /// The open document: the page under the eyes, the name to show and the
    /// outline a chapter label is read out of.
    reader: ReaderState,
    /// The persisted settings: the label's five knobs all live under Layout.
    ...
    let measure = move || {
        request_animation_frame(move || {
            ...
            if reader.viewer.zooming_now() { return; }
            ...
            let page = reader.viewer.page.get_untracked().max(1);
    ...
    Effect::new(move |_| {
        _ = reader.viewer.container_size.get();
        _ = reader.viewer.page.get();
        _ = reader.viewer.mode.get();
        _ = reader.viewer.zoom.transition.get();
        _ = reader.viewer.zoom.display.get();
        _ = reader.document.title.get();
        _ = reader.document.path.get();
        _ = reader.document.outline.get();
        _ = settings.with(|s| {
            (
    ...
    let enabled = move || settings.with(|st| st.layout.floating_label);
    let label = move || {
        use reader_core::settings::FloatingLabelStyle::*;
        let st = settings.with(|s| s.layout.floating_label_style);
        let r = &reader;
        match st {
    ...
    let shown = move || {
        if !enabled() || reader.document.status.get() != DocStatus::Ready {
            return false;
        }
        ...
        if !reader.viewer.first_paint.get() {
            return false;
        }
        ...
        if settings.with(|st| st.layout.floating_label_persist) {
            return rail_off;
        }
        let max_pct = settings.with(|st| st.layout.floating_label_max_pct);
        rail_off
