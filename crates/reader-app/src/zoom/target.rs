...
///
/// Public because the open flow computes a startup scale before any reader
/// exists to ask: it measures the window itself, builds these dims from plain
/// numbers and asks the same `fit` the live pipeline asks, so the first frame
/// is already at the scale a mounted viewer would have settled on.
