...
//! gloss marks. Window chrome (the rail mode, the toast) is NOT here — it is
//! the shell's, and the reader is handed the one signal of it that a zoom
//! watcher needs. Pure domain logic lives in `reader-core` and the format
//! crates.
//!
//! [`ReaderState`] is the whole of what this crate owns. The two things it
//! cannot own — the persisted settings and the library's rows — arrive as
//! props on [`crate::features::root::ReaderRoot`], and the one write it owes
//! storage arrives as the injected [`GlossSave`].
//!
...
pub use gloss::{GlossSave, GlossState, gloss_key};
