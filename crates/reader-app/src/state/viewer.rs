...
// The type lives in `reader-core` (`reader_core::ui`) next to its settings
// input; re-exported so the reader state's callers keep the short path.
pub use reader_core::ui::Motion;
...
pub struct ViewerSignals {
    ...
    /// `crate::effects::page_selection` is the single writer of this signal,
