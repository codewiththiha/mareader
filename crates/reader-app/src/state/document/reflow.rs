...
impl ReflowContent {
    ...
    ///
    /// `prev_page` is the page the reader is on and `zoom_scale` the scale it
    /// is reading at: the two facts about the READER a cut has to be told,
    /// because the cut holds the reader on the block they were reading and
    /// sizes the sheet at the live display scale. Neither is reached for —
    /// this module owns a document, not a window.
    ///
    ...
    pub fn set_initial_heights(
        ...
        self.publish_cut(prev_page, zoom_scale, cuts, geo)
    }
