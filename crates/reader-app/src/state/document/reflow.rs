...
//!   measured heights (`crate::effects::reflow_measure`);
...
impl ReflowContent {
    ...
    /// standing truth, and publish the page cut they produce. `zoom_scale` is
    /// the seed display scale the open flow just initialized; the anchored
    /// page is meaningless at open (the caller chooses the resume page from
    /// the returned cut), so the cut's page is a clamp placeholder.
    ///
    ...
    pub fn set_initial_heights(
        ...
        self.publish_cut(1, zoom_scale, cuts, geo)
    }
    ...
    /// paginated side of the reader. `prev_page` is the page the reader is on
    /// (the anchor the new cut holds); `zoom_scale` is the display scale the
    /// sheet is drawn at.
    ///
    ...
    pub fn recut(
        ...
        Some(self.publish_cut(prev_page, zoom_scale, cuts, geo))
    }
    ...
    /// assemble the answer for the document's page machinery. The reader's
    /// page and zoom arrive as numbers — the anchor page to hold, and the
    /// display scale the sheet is drawn at — because a document deciding its
    /// own pagination should not need the reader around it.
    fn publish_cut(
        &self,
        prev_page: u32,
        zoom_scale: f64,
        ...
        super::ReflowCut {
            ...
            css_height: PAGE_HEIGHT * zoom_scale,
