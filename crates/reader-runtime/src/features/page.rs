...
use crate::features::virtualizers::use_reader_virtualizers;
...
#[component]
pub fn ReaderPage(state: crate::context::ReaderContext) -> impl IntoView {
    ...
    // The runtime handle, kept for the ready mark below. The lifetime is NOT
    // this scope's: the session root that mounted this page begins the mount
    // and registers the disposal cleanup, so the runtime has exactly one
    // owner and one teardown order (§16) — a child scope disposing its
    // parent's runtime would run the engine's destroy against a half-torn
    // tree, and registering the same teardown twice hides which order wins.
    // The context comes from the session too; descendants read it from
    ...
    // Cmd/Ctrl+O) and the AI chunk listener: both act on reader state only,
    // so both moved out of the app root and into this scope, where a library
    ...
    let shell = ShellController::reader(state.chrome);
    ...
    let go_library = { move |_| close_document(&state) };
    let left = move || {
        view! {
            <div
    ...
    let right = move || {
        view! {
            ...
            <AppearanceMenu state=state.chrome />
        }
    };
            ...
            >
                <div class="relative flex min-h-0 flex-1">
                    ...
                    >
                        ...
                        // reusable UI with no knowledge of ReaderContext.
