...
pub(super) fn identity(ctx: &ReaderContext, doc: DocumentIdentity) {
    ...
    let title = match doc.title.as_deref().map(str::trim) {
        ...
        // The LIBRARY's name for the book this open belongs to rides the
        // launch descriptor (the library computed it while it was still
        // active) — a title bar reading "source" for every copied book is
        // the sentence this write exists to prevent.
        _ => ctx
            .launch
...
/// This document's gloss highlights, into a freshly reset gloss ctx. Loaded
...
/// reading it (`storage::migrate_gloss_keys`), so an address read here
