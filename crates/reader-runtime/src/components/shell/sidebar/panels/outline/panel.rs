...
fn reveal_attempt(
    ...
    // The retry chain is a QUEUED callback: a close between two attempts
    // leaves an armed fire whose scope is already gone, and every read and
    // write below would abort the wasm. The slot is the scope's own storage,
    // so a disposed one ends the chain before it touches reader state.
