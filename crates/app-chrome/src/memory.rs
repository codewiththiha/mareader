...
//!
//! It lives in the chrome crate rather than in either app because both halves
//! of the split report it: the shell logs the boot and the close, the reader
//! logs its own search index and zoom commits, and once the reader is its own
//! binary each instance has a linear memory only IT can read — a probe in the
//! shell would report the wrong heap.
//!
//! Off wasm the probe is inert rather than a panic: the wasm-bindgen stubs
//! abort when called natively — the same rule the app crate's clock runs
...
pub fn js_heap_bytes() -> Option<u64> {
    #[cfg(target_arch = "wasm32")]
    {
        use wasm_bindgen::JsValue;
...
pub fn dom_node_count() -> Option<u32> {
    #[cfg(target_arch = "wasm32")]
    {
