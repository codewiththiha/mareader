//! Which of the reader's motions animate, as one projection of the persisted
//! animation settings.
//!
//! This is the reader's format-agnostic motion policy, and it lives beside
//! the settings it is projected from rather than beside the signals it ends
//! up in: the master switch is a persisted decision, the projection is the
//! one place that decision is applied, and every consumer downstream reads
//! the answer instead of asking again. Both the shell's chrome and the
//! reader's surfaces need it, which is why it is here and not in either.
